...
public class GamePlay : Singleton<GamePlay>, IPointerDownHandler, IPointerUpHandler, IBeginDragHandler, IDragHandler
{
        ...
        if (eventData.pointerCurrentRaycast.gameObject != null && !GameBoardGenerator.Instance.DestoryOneBlock)
        {
            ...
            if (clickedObject.GetComponent<ShapeInfo>() != null)
            {
                if (clickedObject.transform.childCount > 0)
                {
                    ...
                    currentShape.blockImage = (DDOL.Theme == 0) ? GamePlayUI.Instance.Light_Theme[10] : GamePlayUI.Instance.Dark_Theme[10];
                    Vector3 pos = Camera.main.ScreenToWorldPoint(eventData.position);
    ...
    void SetImageToPlacingBlocks()
    {
        if (highlightingBlocks != null && highlightingBlocks.Count > 0)
        {
            foreach (Block c in highlightingBlocks)
            {
                c.SetBlockImage(currentShape.ShapeID);
            }
        }
    ...
    void CheckBoardStatus()
    {
        ...
        Move_block_data move_Block_ = new Move_block_data();
        GetComponent<GameBoardGenerator>().move_Block_Data.Add(move_Block_);
        foreach (Block item in highlightingBlocks)
        {
            GetComponent<GameBoardGenerator>().move_Block_Data[GetComponent<GameBoardGenerator>()
                .move_Block_Data.Count - 1].LatestMove_blocks.Add(item.gameObject);
            //print($"Block-{item.rowID}-{item.columnID}");
        }
        highlightingBlocks.Clear();
    ...
    IEnumerator BreakAllCompletedLines(List<List<Block>> breakingRows, List<List<Block>> breakingColumns, int placingShapeBlockCount)
    {
        ...
        else if (TotalBreakingLines == 2)
        {
            ...
            ScoreManager.Instance.AddScore(17 * 20);
        }
        else if (TotalBreakingLines == 3)
        {
            AudioManager.Instance.PlaySound(lineClear3);
            ScoreManager.Instance.AddScore(25 * 20);
        }
        else if (TotalBreakingLines >= 4)
        {
            ...
            {
                ScoreManager.Instance.AddScore(32 * 20);
    ...
    IEnumerator BreakThisLine(List<Block> breakingLine)
    {
        ...
        {
            for (int j = 0; j < GameBoardGenerator.Instance.move_Block_Data.Count; j++)
            {
                for (int k = 0; k < GameBoardGenerator.Instance.move_Block_Data[j].LatestMove_blocks.Count; k++)
                {
                    if (GameBoardGenerator.Instance.move_Block_Data[j].LatestMove_blocks[k].gameObject == b.gameObject)
                    {
                        GameBoardGenerator.Instance.move_Block_Data[j].LatestMove_blocks.Remove(b.gameObject);
                    }
                }
            }
            b.ClearBlock();
